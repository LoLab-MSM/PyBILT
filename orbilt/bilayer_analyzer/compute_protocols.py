...
class ComputeProtocol:
    ...
    def __init__(self, compute_commands):
        ...
        for command in compute_commands:
            if len(command)<2:
                raise RuntimeError("wrong number of arguments for compute "+str(command))
            comp_key = command[0]
            comp_id = command[1]
            comp_args = command[1:]
            if (comp_key in valid_computes):
                if (len(comp_args) >= 1):
                    if comp_id not in compute_ids:
                        ...
                        command_protocol[comp_args[0]]= command_protocols[comp_key](comp_args)
                    else:
                        raise RuntimeError("compute id \'"+comp_id+"\' has already been used")
                else:
                    raise RuntimeError("wrong number of arguments for compute "+str(command))
            else:
                raise RuntimeError("invalid compute \'"+comp_key+"\' :"+str(command))   
        ...
        if (comp_key in valid_computes):
            if (len(comp_args) >= 1):
                if comp_id not in self.compute_ids:
                    ...
                    self.command_protocol[comp_args[0]]= command_protocols[comp_key](comp_args)
                else:
                    raise RuntimeError("compute id \'"+comp_id+"\' has already been used")
            else:
                raise RuntimeError("wrong number of arguments for compute "+str(command))
    ...
    def remove_compute(self, compute_id):
        if compute_id in self.compute_ids:
            ...
            raise RuntimeWarning("no compute with id \'"+compute_id+"\'")
        return
    ...
    def parse_args(self, args):
        ...
        for i in range(1,nargs,2):
            ...
            if arg_key in self.valid_args:
                ...
                raise RuntimeError("ignoring invalid argument key "+arg_key+" for compute "+self.compute_key)
        return 
...
class MSDProtocol(ComputeFunctionProtocol):
    ...
    def parse_args(self, msd_args):
        ...
        for i in range(1,nargs,2):
            ...
            if arg_key in self.valid_args:
                ...
                raise RuntimeError("ignoring invalid argument key "+arg_key+" for compute \'msd\'")
        return 
    #required - a check protocol function which reports relevant settings    
    def print_protocol(self):
        print ("\'"+self.compute_id+"\': msd analysis of "+self.resname+" lipids in "+self.leaflet+" leaflet(s).")
        return
    ...
    def run_compute(self, bilayer_analyzer):
        ...
        count=0
        for i in indices:
            com_curr = bilayer_analyzer.com_frame.lipidcom[i].com_unwrap[bilayer_analyzer.lateral]
        ...
        if self.first_frame:  
            count=0
            for i in indices:
                com_curr = bilayer_analyzer.first_com_frame.lipidcom[i].com_unwrap[bilayer_analyzer.lateral]
...
class APLBoxProtocol(ComputeFunctionProtocol):
    ...
    def __init__(self, apl_args):
        ...
        #parse input arguments if given
        if len(apl_args)>1:
            raise RuntimeWarning("ignoring extra arguments passed to area per lipid compute \'"+self.compute_id+"\'")
    ...
    #required - a check protocol function which reports relevant settings    
    def print_protocol(self):
        print ("\'"+self.compute_id+"\': area per lipid using box dimensions")
        return
...
class BTGridProtocol(ComputeFunctionProtocol):
    ...
    def __init__(self, args):
        ...
        #parse input arguments if given
        if len(args)>1:
            raise RuntimeWarning("ignoring extra arguments passed to bilayer thickness compute \'"+self.compute_id+"\'")
    ...
    #required - a check protocol function which reports relevant settings    
    def print_protocol(self):
        print ("\'"+self.compute_id+"\': bilayer thickness using lipid_grid")
        return
...
class APLGridProtocol(ComputeFunctionProtocol):
    ...
    #required- function to parse the input arguments    
    def parse_args(self, args):
        raise RuntimeWarning("ignoring extra arguments passed to area per lipid (by grid) compute \'"+self.compute_id+"\'")
        return
    #required - a check protocol function which reports relevant settings    
    def print_protocol(self):
    ...
    def run_compute(self, bilayer_analyzer):
        ...
        if interval == self.interval:
            ...
            if self.leaflet == "both":
                ...
                #unknown option--use default "both"
                raise RuntimeWarning("bad setting for \'leaflet\' in "+self.compute_id+". Using default \'both\'")
                self.leaflet = 'both'
                for leaflets in bilayer_analyzer.leaflets:
                    curr_leaf = bilayer_analyzer.leaflets[leaflets]
                    indices+=curr_leaf.get_group_indices(self.group)
            ...
            for i in indices:
                ...
                com_i = curr_frame.lipidcom[i].com_unwrap[bilayer_analyzer.lateral]
                com_j = prev_frame.lipidcom[i].com_unwrap[bilayer_analyzer.lateral]
