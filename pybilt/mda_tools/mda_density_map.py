...
def position_density_map_2d(universe, mda_selections, fstart=0, fend=-1, fstep=1, norm_axis='z', nbins=100,
                            reference=(0.0, 0.0), refsel=None, normalize=False, scale_to_max=False):
    lat_ind = [0, 1]
    if norm_axis is 'x':
        lat_ind = [1, 2]
    elif norm_axis is 'y':
...
def position_density_map_2d_leaflet_simple(universe, bilayer_selection, resnames, fstart=0, fend=-1, fstep=1, norm_axis='z', nbins=100,
                            reference=(0.0, 0.0), refsel=None, normalize=False, scale_to_max=False):
    lat_ind = [0, 1]
    norm_ind = 2
    if norm_axis is 'x':
    ...
    for item in resnames:
        out_counts[item] = dict()
        out_counts[item]['upper'] = np.zeros((nbins,nbins))
        out_counts[item]['lower'] = np.zeros((nbins, nbins))
    for frame in universe.trajectory[fstart:fend:fstep]:

        for item in resnames:
            for leaflet in ['upper', 'lower']:
                selection = None
                z_com = bilayer_selection.atoms.center_of_mass()[norm_ind]

                if leaflet == 'upper':
                    ss = "(resname {}) and (prop {} > {})".format(item, norm_axis, z_com+5.0)
                    #print(ss)
                    selection = universe.select_atoms(ss)
                elif leaflet == 'lower':
                    ss = "(resname {}) and (prop {} < {})".format(item, norm_axis, z_com-5.0)
                    selection = universe.select_atoms(ss)
                ...
                for i in range(len(x_push_index)):
                    ...
                    j += 1
                #counts_f /= binvolume
                out_counts[item][leaflet] += counts_f
        f += 1
    nframes = float(nframes)
    #print(out_counts)
    if normalize:
        for key in out_counts.keys():
            out_counts[key]['upper'] /= out_counts[key]['upper'].sum()
            out_counts[key]['lower'] /= out_counts[key]['lower'].sum()
            #out_counts[key] /= np.max(out_counts[key])
    elif scale_to_max:
        for key in out_counts.keys():
            #out_counts[key] /= out_counts[key].sum()
            out_counts[key]['upper'] /= np.max(out_counts[key]['upper'])
            out_counts[key]['lower'] /= np.max(out_counts[key]['lower'])
    x_centers -= reference[0]
    y_centers -= reference[1]
